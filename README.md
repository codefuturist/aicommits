...
1. Install _aicommits_:
   ...
   **From this fork (includes GitHub Copilot & Mistral providers, `split` command):**
   ...
   > **`--allow-build`** is required by pnpm v10+ security policy so the package can run its `prepare` build script during install (dist/ is not committed to git — it is built from source on install).
